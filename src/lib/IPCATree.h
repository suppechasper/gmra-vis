...
class FixedNodeFactory : public NodeFactory<TPrecision>{
  ...
  public:
    ...
    virtual void createNode(FortranLinalg::DenseMatrix<TPrecision> X){
      ...
      this->radius =  sqrtf(Linalg<TPrecision>::Max(l));
...
class IPCANode : public GMRANodeBase<TPrecision>{
  ...
  public:
    ...
    IPCANode<TPrecision>(FortranLinalg::DenseMatrix<TPrecision> &Xi, std::vector<int>
        ...
        SplitStrategy splitPoint = MEAN, int maxKidDim = 3) : indices(ind){
      ...
      sigma2 = nf.getSigma2();
      mse = nf.getMSE();
      l2Radius = nf.getL2Radius();

      if(splitDir == PC){
        dir = Linalg<TPrecision>::ExtractColumns(phi, 0, std::min( maxKidDim,
              (int) phi.N() ) );
      }
      else if(splitDir == RANDOM_PC){
        static Random<TPrecision> random;
        DenseVector<TPrecision> tmp = DenseVector<TPrecision>(phi.M());
        for(unsigned int i=0; i<dir.N(); i++){
          double w = random.Uniform() * sigma(i);
          Linalg<TPrecision>::AddScale(tmp, w, dir, i, tmp);
        }
      ...
      if(splitPoint == MEAN || splitPoint == RANDOM_MEAN){
        ...
        if(splitPoint == RANDOM_MEAN){
          for(int i=0; i < dir.N() ; i++){
            ...
            static Random<TPrecision> random;
            TPrecision s = random.Normal() * sigma(i)/3.0;
...
class IPCATree : public GMRATree<TPrecision>{
  ...
  public:
    ...
    IPCATree(TPrecision eps, StoppingCriterium stopC, NodeFactory<TPrecision>
        *factory, SplitStrategy ss, int mkd = 3, int minLeafSize=1): stop(stopC),
    epsilon(eps), splitStrategy(ss), minPoints(minLeafSize), nf(factory),
    maxKidDim(mkd)
    { 
